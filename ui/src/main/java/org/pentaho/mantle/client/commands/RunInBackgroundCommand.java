...
public class RunInBackgroundCommand extends AbstractCommand {
  ...
  public RunInBackgroundCommand( RepositoryFile repositoryFile ) {
  ...
  protected  void checkSchedulePermissionAndDialog() {
    final String url = ScheduleHelper.getPluginContextURL() + "api/scheduler/isScheduleAllowed?id=" + repositoryFile.getId(); //$NON-NLS-1$
  ...
  protected void performOperation( String repositoryPath, boolean feedback ) {

    String urlPath = NameUtils.URLEncode( NameUtils.encodeRepositoryPath( repositoryPath ) );
      ...
      scheduleFileRequestBuilder.sendRequest( null, new RequestCallback() {
        ...
        public void onResponseReceived( Request request, Response response ) {
          if ( response.getStatusCode() == Response.SC_OK ) {
            ...
            scheduleRequest.put( "inputFile", new JSONString( repositoryPath ) ); //$NON-NLS-1$
              ...
              emailValidRequest.sendRequest( null, new RequestCallback() {
                ...
                public void onResponseReceived( Request request, Response response ) {
                  if ( response.getStatusCode() == Response.SC_OK ) {
                    ...
                    if ( hasParams ) {
                      boolean isSchedulesPerspectiveActive = !getActivePerspective().equals("schedules.perspective" );
                      createScheduleParamsDialog( repositoryPath, scheduleRequest, isEmailConfValid, isSchedulesPerspectiveActive );
                    } else if ( isEmailConfValid ) {
                      createScheduleEmailDialog( repositoryPath, scheduleRequest );
                    } else {
  ...
  private native String getActivePerspective()
    /*-{
      return $wnd.mantle_getActivePerspectiveId;
    }-*/;

  private static native void setupNativeHooks( RunInBackgroundCommand cmd )
    /*-{
//      $wnd.executeRunInBackground = function(repositoryFilePath,feedback) {
//        cmd.@org.pentaho.mantle.client.commands.RunInBackgroundCommand::performOperation(Ljava/lang/String;Ljava/lang/Boolean;)(repositoryPath, feedback);
//      }
