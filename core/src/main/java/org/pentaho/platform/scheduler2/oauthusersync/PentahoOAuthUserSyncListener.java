...
package org.pentaho.platform.scheduler2.oauthusersync;
...
public class PentahoOAuthUserSyncListener implements IPluginLifecycleListener, IJobFilter {
  ...
  public static final String I_SCHEDULER_2 = "IScheduler2";
  private final Log logger = LogFactory.getLog( PentahoOAuthUserSyncListener.class );

  private static final String RESERVEDMAPKEY_ACTIONUSER = "ActionAdapterQuartzJob-ActionUser";
  ...
  public enum Frequency {
    NOW( "now" ) {
      @Override public IJobTrigger createTrigger() {
        IScheduler scheduler = PentahoSystem.get( IScheduler.class, I_SCHEDULER_2, PentahoSessionHolder.getSession() );
    ...
    ALTERNATE_DAY( "alternate_day" ) {
      @Override public IJobTrigger createTrigger() {
        ...
        // execute alternate days at 0 hours
        IJobTrigger trigger = scheduler.createComplexTrigger( null, null, null, null, 0 );
    ...
    public static final String I_SCHEDULER_2 = PentahoOAuthUserSyncListener.I_SCHEDULER_2;
  ...
  public boolean startup( IPentahoSession session ) {
    IScheduler scheduler = PentahoSystem.get( IScheduler.class, I_SCHEDULER_2, session );
    if ( scheduler == null ) {
  ...
  private void rescheduleIfNecessary( IScheduler scheduler, List<IJob> jobs ) throws SchedulerException {
    ...
    List<IJob> matched = new ArrayList<>( jobs.size() );
    for ( IJob job : jobs ) {
